...
export class DashboardService {
    ...
    getFacilitiesListWithFullDetails(userId, type) {
        return this.http.get(URLConstants.FACILITIES_FULL_DETAILS_URL, this.getFacilitiesOptions(userId, type))
            .map(FacilitiesData => {
    ...
    getFacilityDetails(userId, facilitiesId) {
        return this.http.get(URLConstants.FACILITY_URL, this.getSpecifiFacilityOptions(userId, facilitiesId))
            .map(FacilitiesData => {
    ...
    getFacilitiesForCompany(userId,companyId) {
        return this.http.get(URLConstants.COMPANIES_FACILITIES_URL, this.getFacilitiesForComapnayOptions(userId, companyId))
            .map(companiesData => {
    ...
    addOrUpdateTankMonitorSignup(tankSignup) {
        return this.http.post(URLConstants.TANK_MONITOR_URL, {
            name: tankSignup.name, fid: tankSignup.fid,
    ...
    saveSiteIncome(incomeModel) {
        return this.http.post(URLConstants.SITE_INCOME, {
            accountID: incomeModel.accountID, fID: incomeModel.fID
            ...
            ,cigaretteSalesAmount: incomeModel.cigaretteSalesAmount, lotteryAmount: incomeModel.lotteryAmount, scratchOffSold: incomeModel.scratchOffSold, toDate: incomeModel.toFormatedDate, dataEnteredBy: incomeModel.dataEnteredBy,
    ...
    saveSiteExpenditure(expenditureModel) {
        return this.http.post(URLConstants.SITE_EXPENDITURE, {
            accountID: expenditureModel.accountID, fID: expenditureModel.fID, amount: expenditureModel.amount
    ...
    getIncomeDataByNetIncome(incomeChartModel) {
        return this.http.get(URLConstants.SITE_NET_INCOME, this.getIncomeOptions(incomeChartModel))
            .map(expensesData => {
    ...
    getFacilityReportDirectlyFromStation(userId, fid, facilitiesId, reportType,keyCode) {
        return this.http.get(URLConstants.TANK_RESULT_FROM_STATION_URL, this.getFacilityReportDirectlyFromStationOptions(userId, fid, facilitiesId, reportType,keyCode))
            .map(tankresultData => {

                return tankresultData;
            });
    }
    getFacilityReportDirectlyFromStationWithoutLogin(userId, fid, facilitiesId, reportType,keyCode) {
        return this.http.getReport(URLConstants.TANK_RESULT_FROM_STATION_URL, this.getFacilityReportDirectlyFromStationOptions(userId, fid, facilitiesId, reportType,keyCode))
            .map(tankresultData => {
    ...
    private getSpecifiFacilityOptions(userId, facilitiesId) {
        return {
            params: new HttpParams().set(CRMConstants.USER_ID, userId).set(CRMConstants.SPECIFIC_FACILITY_ID, facilitiesId)
        };
    }
    ...
    private getFacilityReportDirectlyFromStationOptions(userId, fid, facilityId, reportType,keyCode) {
        return {
            params: new HttpParams().set(CRMConstants.USER_ID, userId).set(CRMConstants.FID, fid).set(CRMConstants.FACILITY_ID, facilityId)
            .set(CRMConstants.REPORT_TYPE, reportType).set(CRMConstants.TANK_REPORT_KEY_CODE, keyCode)
        };

    }
    private getFacilitiesForComapnayOptions(userId, companyId) {
        return {
            params: new HttpParams().set(CRMConstants.USER_ID, userId).set(CRMConstants.COMPANY_NAME, companyId)
        };
    }
}
